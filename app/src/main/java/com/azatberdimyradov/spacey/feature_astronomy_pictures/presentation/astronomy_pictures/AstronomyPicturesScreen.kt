...
import com.azatberdimyradov.spacey.feature_astronomy_pictures.presentation.AstronomyPicturesViewModel
        ...
        Column(modifier = Modifier.fillMaxSize()) {
            Row(
                ...
                DateRangePicker(
                    ...
                    viewModel.searchAstronomyPictures(datePair.first, datePair.second)
                }
            }
            LazyColumn {
                items(state.astronomyPictures) { astronomyPictures ->
                    AstronomyPicturesItem(
                        astronomyPictures = astronomyPictures
                    ) { astronomyPictures ->
                        navController
                            .navigate(
                                Screen.ImageDetailsScreen.route + "?image_url=${astronomyPictures.url}&date=${astronomyPictures.date}"
                            )
                    }
                }
            }
